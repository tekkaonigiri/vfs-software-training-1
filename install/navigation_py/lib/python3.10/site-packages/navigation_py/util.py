...
def update_screen(drone_loc: np.ndarray, target_loc: np.ndarray):
    return
    # Identify which point to draw the drone location on
    # Identify which point to draw the target location on
    # loop through and if the i and j indexes are equal to the drone point/
    # target point, change screen icon at that point accordingly
    # else, print -
